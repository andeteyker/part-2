/**
 * Natürliche Linkbegriffe für Rechner innerhalb der Ratgebertexte.
 * Pro Abschnitt wird jeder passende Rechner höchstens einmal verlinkt.
 */
export const guideToolLinkTerms: Record<string, string[]> = {
  "abfindungs-rechner": ["Abfindung"],
  "angebots-kalkulation": ["Gewinnaufschlag", "Selbstkosten", "Angebotspreis"],
  "anhalteweg-rechner": ["Anhalteweg", "Bremsweg", "Reaktionsweg"],
  "arbeitslosengeld-rechner": ["Arbeitslosengeld", "ALG I", "Arbeitslosigkeit"],
  "badrenovierung-rechner": ["Badrenovierung", "Badsanierung", "Renovierungsbudget", "Sicherheitspuffer"],
  "balkonkraftwerk-rechner": ["Balkonkraftwerk", "Steckersolar", "Steckersolaranlage", "Eigenverbrauch"],
  "bauspar-rechner": ["Bausparguthaben", "Bausparziel", "Bausparen"],
  "bmi-rechner": ["BMI"],
  "bodenverlegung-kosten-rechner": ["Bodenfläche", "Bodenverlegung", "Bodenkosten"],
  "co2-heizkosten-rechner": ["CO₂-Kosten", "CO₂-Preis", "CO2-Kosten", "CO2-Preis"],
  "dachkosten-rechner": ["Dachfläche", "Dachsanierung", "Dachkosten"],
  dreisatzrechner: ["Dreisatz"],
  "eauto-verbrenner-kosten-rechner": ["E-Auto", "Elektroauto", "Verbrenner", "Ladekosten"],
  "einkommensteuer-rechner": ["Einkommensteuer", "Tarifsteuer"],
  "elterngeld-rechner": ["Elterngeld"],
  "energie-abschlag-rechner": ["Abschlag", "Stromabschlag", "Gasabschlag", "Energieabschlag"],
  "erhaltungsruecklage-rechner": ["Instandhaltungsrücklage", "Erhaltungsrücklage"],
  "erspartes-reichweite-rechner": ["Liquiditätsreserve", "Notfallreserve", "Erspartes"],
  "fahrtkosten-rechner": ["Fahrtkosten", "Autokosten", "Kraftstoffkosten", "Spritkosten"],
  "gasverbrauch-rechner": ["Gasverbrauch", "Kubikmeter Gas", "Kubikmeter"],
  "haus-leisten-rechner": ["Kaufpreislimit", "Immobilienbudget", "Kaufbudget"],
  "haushaltsbudget-rechner": ["Haushaltsbudget", "Monatsbudget", "Budgetplanung", "freies Budget", "monatliche Belastung", "Haushaltsrechnung"],
  "haushaltsgeraete-kosten-rechner": ["Haushaltsgerät", "Energielabel", "Gerätekosten", "Stromverbrauch eines Geräts"],
  "heizkosten-rechner": ["Heizkosten"],
  "idealgewicht-rechner": ["Gewichtsbereich", "Idealgewicht"],
  "kalorienbedarf-rechner": ["Kalorienbedarf", "Grundumsatz", "Gesamtbedarf"],
  "kaufnebenkosten-rechner": ["Kaufnebenkosten", "Erwerbsnebenkosten"],
  "kindergeld-rechner": ["Kindergeld", "Monatsbetrag", "Gesamtsumme"],
  "koerperfett-anteil-rechner": ["Körperfettwert", "Körperfett"],
  "kredit-sondertilgung-rechner": ["Sondertilgung", "Sondertilgungen", "Zinsersparnis", "Restschuld nach der Zinsbindung"],
  "leasingfaktor-rechner": ["Leasingfaktor", "Leasingrate", "Leasingangebot"],
  mehrwertsteuerrechner: ["Umsatzsteuer", "Mehrwertsteuer"],
  "mietkaution-rechner": ["Mietkaution", "Kaution", "Nettokaltmiete"],
  "mietrendite-rechner": ["Nettorendite", "Bruttorendite", "Mietrendite"],
  "nachtzuschlag-rechner": ["Nachtzuschlag", "Nachtstunden"],
  "nebenkosten-vorauszahlung-rechner": ["Nebenkostenvorauszahlung", "Betriebskostenvorauszahlung", "Vorauszahlungen", "Nebenkosten", "Abschlagszahlung"],
  "netto-gehalt-rechner": ["Nettogehalt", "Netto"],
  "pendlerpauschale-rechner": ["Pendlerpauschale", "Entfernungspauschale"],
  "powerstation-laufzeit-rechner": ["Geräteleistung", "Nutzungsdauer", "Wattstunden"],
  prozentrechner: ["Prozentwert", "Prozent"],
  rentenrechner: ["Bruttorente", "Rentenpunkte", "Rente"],
  "schichtlohn-rechner": ["Schichtlohn", "Grundvergütung"],
  "schwangerschafts-terminrechner": ["Geburtstermin", "Schwangerschaftswoche"],
  "selbststaendig-stundensatz-rechner": ["Stundensatz für Selbstständige", "Freelancer-Stundensatz", "abrechenbare Stunden", "betrieblicher Stundensatz", "produktive Arbeitsstunden"],
  "skonto-rechner": ["Skonto", "Zahlungsziel", "Zahlungsbedingungen"],
  "solar-ertrag-rechner": ["Jahresertrag", "Solarertrag", "Photovoltaikanlage"],
  "stromkosten-rechner": ["Stromkosten", "Kilowattstunden pro Jahr"],
  stundenlohnrechner: ["Stundenlohn", "Stundensatz"],
  "ueberstunden-rechner": ["Überstunden", "Freizeitausgleich"],
  "umzugskosten-rechner": ["Umzugskosten", "Umzugsbudget", "Eigenumzug", "Firmenangebot"],
  "urlaubsanspruch-rechner": ["Urlaubsanspruch", "Urlaubstage", "Tage Urlaub", "Jahresurlaub", "Teilurlaub"],
  "werbungskosten-steuerersparnis-rechner": ["Werbungskostenbetrag", "Werbungskosten", "Steuerwirkung"],
  "woerter-zaehlen": ["Wortzahl", "Lesezeit"],
  "zeichen-zaehlen": ["Zeichenzahl", "Zeichen"],
  "zinseszins-rechner": ["Zinseszins", "Sparplan", "Rendite"],
};
