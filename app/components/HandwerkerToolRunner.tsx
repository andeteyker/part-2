...
const fmt = (value: number, digits = 1) => new Intl.NumberFormat("de-DE", { maximumFractionDigits: digits }).format(value || 0);

function Field({ label, value, onChange, suffix, step = "0.01" }: { label: string; value: string; onChange: (value: string) => void; suffix?: string; step?: string }) {
  return <label className="field"><span>{label}</span><div><input type="number" min="0" step={step} value={value} onChange={(e) => onChange(e.target.value)} />{suffix && <b>{suffix}</b>}</div></label>;
}

function Total({ label, value, detail }: { label: string; value: number; detail?: string }) {
  return <div className="result-box"><span>{label}</span><strong>{money(value)}</strong>{detail && <small>{detail}</small>}</div>;
}
...
function PaintingCostTool() {
  ...
  return <><div className="field-grid two"><Field label="Zu streichende Fläche" value={wallArea} onChange={setWallArea} suffix="m²" /><Field label="Arbeits-/Komplettpreis pro m²" value={unitPrice} onChange={setUnitPrice} suffix="€/m²" /><Field label="Vorarbeiten (Spachteln, Abkleben etc.)" value={prep} onChange={setPrep} suffix="€" /><Field label="Zusätzliches Material" value={material} onChange={setMaterial} suffix="€" /></div><div className="stats-grid three"><div><strong>{money(num(wallArea) * num(unitPrice))}</strong><span>Flächenkosten</span></div><div><strong>{money(num(prep))}</strong><span>Vorarbeiten</span></div><div><strong>{money(num(material))}</strong><span>Material</span></div></div><Total label="Geschätzte Malerkosten" value={total} /><p className="form-hint">Für einen Angebotsvergleich solltest du prüfen, ob Abdeckarbeiten, Grundierung, Spachtelarbeiten, Farbe und Anfahrt im jeweiligen Einheitspreis enthalten sind.</p></>;
}
