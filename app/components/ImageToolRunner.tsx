...
function ResizeTool() {
  ...
  const select = async (next: File[]) => { setFiles(next); setOutput(null); if (next[0]) try { const raster = await loadRaster(next[0]); setWidth(String(raster.width)); setHeight(String(raster.height)); setRatio(raster.width / raster.height); } catch { setError("Die Bildabmessungen konnten nicht gelesen werden."); } };
  const changeWidth = (value: string) => { setWidth(value); const parsed = Number(value); if (locked && parsed > 0) setHeight(String(Math.max(1, Math.round(parsed / ratio)))); }; const changeHeight = (value: string) => { setHeight(value); const parsed = Number(value); if (locked && parsed > 0) setWidth(String(Math.max(1, Math.round(parsed * ratio)))); };
  const run = async () => { const nextWidth = Number(width), nextHeight = Number(height); if (!file) return; if (!width || !height || nextWidth < 1 || nextHeight < 1) { setError("Bitte gib eine gültige Breite und Höhe ein."); return; } setBusy(true); setError(""); try { const raster = await loadRaster(file); const canvas = canvasFrom(raster.image, nextWidth, nextHeight); const blob = await canvasBlob(canvas, "image/png"); setOutput({ blob, name: `${baseName(file.name)}-${nextWidth}x${nextHeight}.png`, width: nextWidth, height: nextHeight }); } catch { setError("Das Bild konnte nicht skaliert werden."); } finally { setBusy(false); } };
  return <ToolShell><UploadZone files={files} onFiles={select} />{file && <div className="image-controls"><div className="dimension-grid"><label><span>Breite in Pixeln</span><input type="number" min="1" max="12000" value={width} required aria-invalid={!width || undefined} onFocus={(event) => event.currentTarget.select()} onChange={(event) => changeWidth(event.target.value)} /></label><label><span>Höhe in Pixeln</span><input type="number" min="1" max="12000" value={height} required aria-invalid={!height || undefined} onFocus={(event) => event.currentTarget.select()} onChange={(event) => changeHeight(event.target.value)} /></label></div><label className="check-row"><input type="checkbox" checked={locked} onChange={(event) => setLocked(event.target.checked)} /> Seitenverhältnis beibehalten</label><button className="primary-button" onClick={run} disabled={busy}>{busy ? "Größe wird geändert …" : "Neue Bildgröße erstellen"}</button></div>}<ErrorNote error={error} />{output && <DownloadResult output={output} originalSize={file?.size} />}</ToolShell>;
}
