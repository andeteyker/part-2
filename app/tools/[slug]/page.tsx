...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Script id={`schema-${tool.slug}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><SiteHeader /><main className="tool-page"><div className="shell"><nav className="breadcrumbs" aria-label="Brotkrümelnavigation"><Link href="/">Startseite</Link><span>›</span><Link href="/#tools">{tool.category}</Link><span>›</span><b>{tool.title}</b></nav><section className="tool-hero"><p className="eyebrow"><span /> {tool.eyebrow}</p><h1>{tool.title}</h1><p>{tool.description}</p><div className="tool-badges"><span>✓ Kostenlos</span><span>✓ Ohne Anmeldung</span><span>✓ Sofortiges Ergebnis</span></div></section><ToolRunner slug={tool.slug} /><section className="content-grid"><article><p className="eyebrow"><span /> Anleitung</p><h2>So funktioniert: {tool.title}</h2><ol><li><b>Werte eingeben</b><span>Trage die benötigten Angaben in die übersichtlichen Felder ein.</span></li><li><b>Ergebnis erhalten</b><span>Das Werkzeug berechnet oder verarbeitet deine Eingabe direkt.</span></li><li><b>Ergebnis verwenden</b><span>Übernimm das Ergebnis oder lade die erzeugte Datei herunter.</span></li></ol></article><aside><h2>Häufige Fragen</h2>{tool.faq.map((item) => <details key={item.question}><summary>{item.question}</summary><p>{item.answer}</p></details>)}</aside></section>{related.length > 0 && <section className="related"><div className="section-head"><div><p className="eyebrow"><span /> Gleiche Nische</p><h2>Weitere Tools für {tool.category}</h2></div><Link href="/#tools">Alle Nischen →</Link></div><div className="tool-grid compact">{related.map((item) => <Link href={`/tools/${item.slug}`} className="tool-card" key={item.slug}><div className="card-top"><span className="tool-icon">{item.icon}</span><span className="tool-arrow">↗</span></div><p>{item.eyebrow}</p><h3>{item.title}</h3><span>{item.short}</span></Link>)}</div></section>}</div></main><SiteFooter /></>;
}
